...
## 🗜️ WASM Binary Size Optimization

> Addressed in [#1612](https://github.com/Gildado/PayD/issues/1612) — Mainnet launch readiness.

All Soroban contracts in this repository are compiled with the following size-reduction profile settings (defined once in the workspace `Cargo.toml`):

| Setting | Value | Effect |
|---|---|---|
| `opt-level` | `"z"` | Prioritize binary size over speed in all crates |
| `lto` | `true` | Whole-program link-time optimization removes dead code |
| `codegen-units` | `1` | Single CGU allows maximum cross-function inlining and DCE |
| `panic` | `"abort"` | Removes Rust's unwinding machinery (~5–15 KB per contract) |
| `strip` | `true` | Strips debug symbols and section names from WASM output |
| `debug` | `false` | No DWARF debug sections embedded in release builds |
| `debug-assertions` | `false` | Removes bounds-check debug paths |
| `build-override.opt-level` | `"z"` | Build scripts and proc-macros also compiled for size |
| `build-override.codegen-units` | `1` | Consistent cross-function DCE in macro expansion |

### Building optimized contracts
...
The workspace profile is applied automatically — no per-contract flags are needed.
...
To further reduce size, consider running `wasm-opt -Oz` (from the `binaryen` toolchain) as a post-processing step.
