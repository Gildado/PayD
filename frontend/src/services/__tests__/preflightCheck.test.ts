...
describe('checkAccountExists', () => {
  it('returns true when account exists', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${HORIZON_URL}/accounts/GABCDEF`, {
...
describe('checkBalance', () => {
  it('returns true when native balance meets minimum', async () => {
    mockFetch.mockResolvedValueOnce(
      createJsonResponse({
        id: 'GABCDEF',
        balances: [{ balance: '5.0000000', asset_type: 'native' }],
      })
    );
  ...
  it('returns false when native balance is below minimum', async () => {
    mockFetch.mockResolvedValueOnce(
      createJsonResponse({
        id: 'GABCDEF',
        balances: [{ balance: '0.5000000', asset_type: 'native' }],
      })
    );
    const result = await checkBalance('GABCDEF', '1', HORIZON_URL);
    expect(result).toBe(false);
  ...
  it('returns false when account has no native balance entry', async () => {
    mockFetch.mockResolvedValueOnce(
      createJsonResponse({
        id: 'GABCDEF',
        balances: [{ balance: '100.0000000', asset_type: 'credit_alphanum4', asset_code: 'USDC' }],
      })
    );
    const result = await checkBalance('GABCDEF', '1', HORIZON_URL);
    expect(result).toBe(false);
...
describe('runPreflightChecks', () => {
  const batch: PreflightCheckEmployee[] = [
    {
      name: 'Alice',
      walletAddress: 'GAICE3EYV3KGI7ND4GM4J3K4Y5K6J7K8L9M0N1O2P3Q4R5S6T7U8V9W0X',
      amount: '100',
      currency: 'USDC',
    },
    {
      name: 'Bob',
      walletAddress: 'GBOB4EYV3KGI7ND4GM4J3K4Y5K6J7K8L9M0N1O2P3Q4R5S6T7U8V9W0X',
      amount: '200',
      currency: 'XLM',
    },
    {
