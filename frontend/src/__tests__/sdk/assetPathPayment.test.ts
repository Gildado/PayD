/**
 * Asset Path Payment Contract — SDK Interoperability Tests
 */
...
const WASM_PATH = resolve(
  __dirname,
  '../../../../target/wasm32-unknown-unknown/release/asset_path_payment.wasm'
);
...
describe('AssetPathPayment Contract SDK Interop', () => {
  beforeAll(async () => {
    ...
    console.log(`Deployed asset_path_payment: ${contractId}`);
  });
  ...
  it('returns contract name', async () => {
    ...
    expect(scValToNative(simSuccess.result!.retval)).toBe('asset_path_payment');
  });
