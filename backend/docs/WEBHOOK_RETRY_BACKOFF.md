...
- Report delivery channels use the shared retry helper. Email retries transient SMTP errors such as `421`, `450`, `451`, `452`, timeouts, throttling, and temporary failures. Webhook retries connection errors plus HTTP `429` and `5xx` responses.
- Per-channel `retryPolicy` settings on `DeliveryConfig` control `maxRetries`, `backoffMs`, and `backoffMultiplier`.
