...
// ══════════════════════════════════════════════════════════════════════════════
// #1617 — Multi-tenant state isolation (revenue_split)
// Verify TotalDistributed and LastDistributeLedger are isolated per-token
// and per-distribution cycle.
// ══════════════════════════════════════════════════════════════════════════════

/// TotalDistributed for token A must not appear in token B's counter.
#[test]
fn test_total_distributed_is_per_token_isolated() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Register two separate token contracts.
    let token_admin_a = Address::generate(&env);
    let token_admin_b = Address::generate(&env);
    let (token_a, sac_a, _) = create_token_contract(&env, &token_admin_a);
    let (token_b, _, _) = create_token_contract(&env, &token_admin_b);
    ...
    // Distribute only token A.
    client.distribute(&token_a, &sender, &10_000);

    // Token A counter should be 10_000.
    assert_eq!(
        client.get_total_distributed(&token_a),
        10_000,
        "token A total distributed must be 10_000"
    );

    // Token B was never distributed — its counter must remain zero.
    assert_eq!(
        client.get_total_distributed(&token_b),
        0,
        "token B total distributed must be 0 when only token A was distributed"
    );
}

/// LastDistributeLedger (replay guard) must be shared globally, not per-tenant.
/// Two successive calls in the same ledger sequence should fail (replay detected),
/// and advancing the ledger must allow a second call.
#[test]
fn test_replay_guard_across_ledger_boundaries() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let token_admin = Address::generate(&env);
    let (token_a, sac_a, _) = create_token_contract(&env, &token_admin);
    sac_a.mint(&sender, &1_000_000);
    ...
    // Start at a non-zero sequence (sequence 0 is exempted from replay protection).
    env.ledger().set_sequence_number(10);

    // First distribution succeeds.
    client.distribute(&token_a, &sender, &1_000);

    // Second distribution in the SAME ledger must fail with LedgerReplayDetected.
    let result = client.try_distribute(&token_a, &sender, &1_000);
    assert_eq!(
        result,
        Err(Ok(RevenueSplitError::LedgerReplayDetected)),
        "duplicate distribution in same ledger must be rejected"
    );

    // Advancing the ledger allows a new distribution.
    ...
    assert_eq!(
        client.get_total_distributed(&token_a),
        2_000,
        "total distributed must reflect both successful distributions"
    );
}

// ══════════════════════════════════════════════════════════════════════════════
// #1613 — Native XLM vs custom token uniform handling (revenue_split)
// Verify that distribute() behaves identically for both token types.
// ══════════════════════════════════════════════════════════════════════════════

/// distribute() must work correctly when using a native-style SAC (XLM analogue).
#[test]
fn test_distribute_with_native_xlm_sac() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Register a "native" token (second SAC — same API as custom token in Soroban tests).
    let native_admin = Address::generate(&env);
    let (native_token, native_sac, native_client) = create_token_contract(&env, &native_admin);
    ...
    // Distribute native-style SAC token.
    client.distribute(&native_token, &sender, &10_000);

    // Both recipients must receive the correct split.
    assert_eq!(native_client.balance(&r1), 6_000, "r1 must receive 60% of 10_000 from native SAC");
    assert_eq!(native_client.balance(&r2), 4_000, "r2 must receive 40% of 10_000 from native SAC");

    assert_eq!(
        client.get_total_distributed(&native_token),
        10_000,
        "total distributed counter must include native SAC distribution"
    );
}

/// distribute() must produce identical split ratios for a custom token and a native SAC.
#[test]
fn test_custom_and_native_token_produce_identical_splits() {
    let env = Env::default();
    env.mock_all_auths();

    let admin = Address::generate(&env);
    let sender = Address::generate(&env);
    let r1_custom = Address::generate(&env);
    let r2_custom = Address::generate(&env);
    let r1_native = Address::generate(&env);
    let r2_native = Address::generate(&env);

    let custom_admin = Address::generate(&env);
    let native_admin_addr = Address::generate(&env);
    let (custom_token, custom_sac, custom_client) = create_token_contract(&env, &custom_admin);
    let (native_token, native_sac, native_client) = create_token_contract(&env, &native_admin_addr);

    custom_sac.mint(&sender, &1_000_000);
    native_sac.mint(&sender, &1_000_000);

    // Contract A — splits to custom recipients.
    let contract_id_a = env.register(RevenueSplitContract, ());
    let client_a = RevenueSplitContractClient::new(&env, &contract_id_a);
    let shares_custom = Vec::from_array(
        &env,
        [
            RecipientShare { destination: r1_custom.clone(), basis_points: 7000 },
            RecipientShare { destination: r2_custom.clone(), basis_points: 3000 },
        ],
    );
    client_a.init(&admin, &shares_custom);
    client_a.distribute(&custom_token, &sender, &10_000);

    // Contract B — same split ratios, native token recipients.
    let contract_id_b = env.register(RevenueSplitContract, ());
    let client_b = RevenueSplitContractClient::new(&env, &contract_id_b);
    let shares_native = Vec::from_array(
        &env,
        [
            RecipientShare { destination: r1_native.clone(), basis_points: 7000 },
            RecipientShare { destination: r2_native.clone(), basis_points: 3000 },
        ],
    );
    client_b.init(&admin, &shares_native);
    client_b.distribute(&native_token, &sender, &10_000);

    // Verify split is identical for both token types.
    assert_eq!(custom_client.balance(&r1_custom), native_client.balance(&r1_native),
        "70% recipient must receive same amount regardless of token type");
    assert_eq!(custom_client.balance(&r2_custom), native_client.balance(&r2_native),
        "30% recipient must receive same amount regardless of token type");
}
