...
fn test_daily_limit_boundary_conditions() {
    ...
    let result = client.try_execute_batch(&sender, &token, &payments3, &1);
    assert!(result.is_err());
}
...
fn test_weekly_daily_limit_interaction() {
    ...
    env.ledger().set_sequence_number(LEDGERS_PER_DAY + 1);
    let mut payments2: Vec<PaymentOp> = Vec::new(&env);
    payments2.push_back(PaymentOp {
        recipient: Address::generate(&env),
        amount: 500,
    ...
    env.ledger().set_sequence_number(2 * LEDGERS_PER_DAY + 1);
    let mut payments3: Vec<PaymentOp> = Vec::new(&env);
    payments3.push_back(PaymentOp {
        recipient: Address::generate(&env),
        amount: 500,
    ...
    env.ledger().set_sequence_number(3 * LEDGERS_PER_DAY + 1);
    ...
    let result = client.try_execute_batch(&sender, &token, &payments4, &3);
    assert!(result.is_err());
...
fn test_monthly_limit_overarching_cap() {
    let (env, sender, token, client) = setup();
    // Set limits: no daily cap, no weekly cap, monthly cap of 5_000
    client.set_default_limits(&0, &0, &5_000);
    const LEDGERS_PER_DAY: u32 = 17_280;

    // Spend500 per day for 10 days = 5_000 (monthly cap)
    for i in 0u64..10 {
        env.ledger().set_sequence_number((i as u32) * LEDGERS_PER_DAY + 1);
        let mut payments: Vec<PaymentOp> = Vec::new(&env);
        payments.push_back(PaymentOp {
            recipient: Address::generate(&env),
            amount: 500,
    ...
    env.ledger().set_sequence_number(10 * LEDGERS_PER_DAY + 1);
    let mut payments: Vec<PaymentOp> = Vec::new(&env);
    payments.push_back(PaymentOp {
        recipient: Address::generate(&env),
        amount: 1,
    ...
    let result = client.try_execute_batch(&sender, &token, &payments, &10);
    assert!(result.is_err());
}
...
fn test_limit_tracking_across_multiple_batches() {
    ...
    let result = client.try_execute_batch(&sender, &token, &p4, &3);
    assert!(result.is_err());
...
fn test_failed_payments_do_not_count_toward_limits() {
    ...
    let result2 = client.try_execute_batch(&sender, &token, &payments2, &1);
...
fn test_admin_decrease_limits_mid_period_blocks_spending() {
    ...
    let result = client.try_execute_batch(&sender, &token, &payments2, &1);
    assert!(result.is_err());
...
fn test_weekly_limit_reset_at_boundary() {
    ...
    env.ledger().set_sequence_number(120_960);
...
fn test_monthly_limit_reset_at_boundary() {
    ...
    env.ledger().set_sequence_number(518_400);
...
// ══════════════════════════════════════════════════════════════════════════════
// #1617 — Multi-tenant state isolation
// Verify that per-account storage (limits, usage) is scoped to the owning
// address and cannot be read or mutated across tenant boundaries.
// ══════════════════════════════════════════════════════════════════════════════

/// AcctLimits for tenant A must not bleed into tenant B's effective limits.
#[test]
fn test_per_tenant_limits_do_not_leak() {
    let (env, _, token, client) = setup();

    let tenant_a = Address::generate(&env);
    let tenant_b = Address::generate(&env);

    // Mint tokens for both tenants.
    soroban_sdk::token::StellarAssetClient::new(&env, &token).mint(&tenant_a, &1_000_000);
    soroban_sdk::token::StellarAssetClient::new(&env, &token).mint(&tenant_b, &1_000_000);

    // Give tenant A a very high daily limit.
    client.set_account_limits(&tenant_a, &500_000, &0, &0);

    // Tenant B should still see the default limits (0 = unlimited here).
    let limits_b = client.get_account_limits(&tenant_b);
    // Tenant B did not receive an override — its limits must differ from A's.
    assert_ne!(
        limits_b.daily_limit,
        500_000,
        "tenant B must not inherit tenant A's limit override"
    );
}

/// AcctUsage incremented for tenant A must be zero for tenant B.
#[test]
fn test_per_tenant_usage_isolation() {
    let (env, _, token, client) = setup();
    ...
    // Tenant A executes a batch — this increments A's usage counters.
    client.execute_batch(&tenant_a, &token, &payments, &0);

    let usage_a = client.get_account_usage(&tenant_a);
    assert_eq!(usage_a.daily_spent, 10_000, "tenant A daily usage should be 10_000");

    // Tenant B has never sent anything — its usage must be zero.
    let usage_b = client.get_account_usage(&tenant_b);
    assert_eq!(
        usage_b.daily_spent, 0,
        "tenant B daily usage must not be affected by tenant A's transactions"
    );
    assert_eq!(
        usage_b.weekly_spent, 0,
        "tenant B weekly usage must not be affected by tenant A"
    );
    assert_eq!(
        usage_b.monthly_spent, 0,
        "tenant B monthly usage must not be affected by tenant A"
    );
}

/// Setting limits for tenant A then removing them does NOT affect tenant B.
#[test]
fn test_remove_tenant_a_limits_leaves_tenant_b_unchanged() {
    let (env, _, token, client) = setup();
    ...
    // Set a custom limit for A, then remove it.
    client.set_account_limits(&tenant_a, &123_456, &0, &0);
    client.remove_account_limits(&tenant_a);

    // Tenant B limits must be independent and unchanged.
    let limits_b = client.get_account_limits(&tenant_b);
    assert_ne!(
        limits_b.daily_limit, 123_456,
        "removing tenant A's limits must not set tenant B's limit to A's old value"
    );
}

/// LastBatchLedger replay protection is per-sender: tenant A triggering replay
/// protection must not block tenant B in the same ledger.
#[test]
fn test_replay_protection_is_per_tenant() {
    let (env, _, token, client) = setup();
    ...
    // Both tenants can execute within the same test (different in-env addresses).
    // A executes first.
    client.execute_batch(&tenant_a, &token, &payments_a, &0);

    // Advance one ledger so B is not replay-blocked by A's ledger record.
    env.ledger().with_mut(|l| l.sequence_number += 1);

    // B should succeed independently.
    let batch_id_b = client.execute_batch(&tenant_b, &token, &payments_b, &1);
    assert!(batch_id_b > 0, "tenant B batch must succeed independently of tenant A");
}

// ══════════════════════════════════════════════════════════════════════════════
// #1613 — Native XLM vs custom token uniform handling (bulk_payment)
// Verify execute_batch and execute_batch_v2 behave identically whether the
// token is a custom SAC or the native XLM SAC.
// ══════════════════════════════════════════════════════════════════════════════

/// Helper: create a second SAC to act as the "native XLM" analogue in tests.
/// In Soroban test environments there is no real native XLM distinction —
/// both are SAC contracts registered via register_stellar_asset_contract_v2.
/// This tests that the contract makes no assumptions about the token type.
fn setup_native_sac(env: &Env) -> (Address, soroban_sdk::token::StellarAssetClient<'static>) {
    let token_admin = Address::generate(env);
    let contract_id = env
        .register_stellar_asset_contract_v2(token_admin.clone())
        .address();
    (
        contract_id.clone(),
        soroban_sdk::token::StellarAssetClient::new(env, &contract_id),
    )
}

/// execute_batch with native-style SAC must produce the same result as with a
/// custom token SAC — recipients receive the correct amounts.
#[test]
fn test_execute_batch_native_xlm_sac() {
    let (env, _, _, client) = setup();

    let sender = Address::generate(&env);
    let (native_token, native_sac) = setup_native_sac(&env);
    native_sac.mint(&sender, &1_000_000);
    ...
    let batch_id = client.execute_batch(&sender, &native_token, &payments, &0);
    assert!(batch_id > 0, "batch_id must be assigned");

    let tc = soroban_sdk::token::Client::new(&env, &native_token);
    assert_eq!(tc.balance(&r1), 500, "r1 must receive 500 from native SAC");
    assert_eq!(tc.balance(&r2), 300, "r2 must receive 300 from native SAC");
}

/// execute_batch_v2 (strict mode) with native SAC must record correct batch state.
#[test]
fn test_execute_batch_v2_strict_native_xlm_sac() {
    let (env, _, _, client) = setup();
    ...
    let batch_id = client.execute_batch_v2(&sender, &native_token, &payments, &client.get_sequence(), &true);

    let tc = soroban_sdk::token::Client::new(&env, &native_token);
    assert_eq!(tc.balance(&r1), 1_000, "recipient must receive 1_000 from native SAC (strict mode)");
    let record = client.get_batch(&batch_id);
    assert_eq!(record.total_sent, 1_000);
    assert_eq!(record.success_count, 1);
    assert_eq!(record.fail_count, 0);
...
/// Mixing two different SACs (custom and native-style) in separate batches
/// on the same contract must account for each independently.
#[test]
fn test_custom_token_and_native_sac_independent_batches() {
    let (env, sender, custom_token, client) = setup();
    ...
    let mut p_custom: Vec<PaymentOp> = Vec::new(&env);
    p_custom.push_back(PaymentOp {
        recipient: r1.clone(),
        amount: 200,
        category: soroban_sdk::symbol_short!("payroll"),
    });

    let mut p_native: Vec<PaymentOp> = Vec::new(&env);
    p_native.push_back(PaymentOp {
        recipient: r2.clone(),
        amount: 400,
        category: soroban_sdk::symbol_short!("payroll"),
    });

    // Execute with custom token.
    let _batch_custom = client.execute_batch(&sender, &custom_token, &p_custom, &0);

    // Advance ledger to avoid replay protection.
    env.ledger().with_mut(|l| l.sequence_number += 1);

    // Execute with native-style SAC.
    let _batch_native = client.execute_batch(&sender, &native_token, &p_native, &1);

    let tc_custom = soroban_sdk::token::Client::new(&env, &custom_token);
    let tc_native = soroban_sdk::token::Client::new(&env, &native_token);

    assert_eq!(tc_custom.balance(&r1), 200, "r1 must receive custom token");
    assert_eq!(tc_native.balance(&r2), 400, "r2 must receive native SAC token");
}
